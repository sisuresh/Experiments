...
Authoring a host CAP: declare new host fns in `soroban-env-common/env.json` (regenerates the `Env` trait), implement bodies in `soroban-env-host`. **Gate the CAP's code on the umbrella `next` feature (`#[cfg(feature = "next")]`) — do NOT mint a per-CAP cargo feature.** Reason: core compiles a *separate* `soroban-env-host-p{N}` per protocol (each pinned to its own rev) and dispatches by ledger protocol version, so "which protocol has the CAP" is already fixed by the pinned rev + `next`; a per-CAP feature is redundant and creates `cap↔next` wiring/cycle problems plus per-consumer drift (core would have to enumerate every cap). The only thing that genuinely needs a compile-time gate is code naming an XDR variant absent from `curr` (e.g. `ScAddress::MuxedContract`), and `next` already means exactly "build against next XDR, where that variant exists." So make `next` pull in the XDR variants it needs — in `soroban-env-common/Cargo.toml`: `next = ["soroban-env-macros/next", "stellar-xdr/cap_<n>_<title>"]`, and forward `next` through host/guest (`next = ["soroban-env-common/next"]`). Then `--features next` = proto-N meta + all its CAPs, in env AND in core (whose `unified`/build enables `soroban-env-host-p{N}/next`, not each cap by name) — nothing to enumerate, no cycle. At release, `next`→`curr`: drop the `#[cfg(feature = "next")]` wrappers and the code goes unconditional. Still gate per-fn availability at runtime via `min_supported_protocol` / `check_protocol_version_lower_bound`. (The per-cap `#ifdef CAP_<n>_<TITLE>` in stellar-xdr and its `cap_<n>` crate feature still exist — that's the XDR layer materializing the variant; only the *env code gate* drops the per-cap feature in favor of `next`.)
