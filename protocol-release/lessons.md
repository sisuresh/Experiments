...
Authoring a host CAP: declare new host fns in `soroban-env-common/env.json` (regenerates the `Env` trait), implement bodies in `soroban-env-host`. **Gate the CAP's code on its OWN cap-specific cargo feature (e.g. `#[cfg(feature = "cap_0084_muxed_contract")]`), NOT the umbrella `next` feature.** Reason: a single CAP can then be pulled before a release by keeping just its gate, without disturbing the other unreleased features. Define the feature in the crate's `Cargo.toml` (forwarding to the matching `stellar-xdr` cap feature) and name it to match the descriptive XDR flag (cf. the stellar-xdr feature-flag note). Still gate at runtime per-fn via `min_supported_protocol` / `check_protocol_version_lower_bound`.
