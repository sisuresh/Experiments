...
- **Stay strictly in scope — no drive-by "improvements."** Change only what the CAP + its required propagation demand. Do NOT "fix" unrelated comments, docs, dependency advisories, lint nits, or anything you happen to notice — even when it looks like a genuine bug. Real regressions this caused: an automation flipped a correct `rentFeeCharged` comment in `Stellar-ledger.x` to a *wrong* xref (contradicting the neighboring field's own comment) inside a CAP-0084 PR; and env/sdk burned ~8 commits chasing out-of-scope cargo-deny advisories (anyhow RUSTSEC, ed25519-dalek), cascading re-pins down the stack. Out-of-scope edits pollute the diff, mislead reviewers, can be flat wrong, and (for deps) trigger churn. If you spot a genuine unrelated issue, note it in the PR description for a human — never edit it.
